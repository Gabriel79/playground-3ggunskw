...
`External::Archive::File m_files[NB_FILES];` définit un tableau dont chaque élément sera un `External::Archive::File` où `File` est le nom de la structure et `External::Archive` le nom de son namespace (Omega n'a pas conservé la convention du code source de la Numworks qui écrit les noms de namespace en minuscule). Pour pouvoir utiliser cette classe, il faut inclure le header qui le définit :\
